...
int pushToSet(NumericSet *ns, int num) {
	...
	if ((ns == NULL) || (num <= 0) || (ns->numbers == NULL) || (ns->real_size < 0) || (ns->max_size <= 0)) {
		return WRONG_INPUT;
	}
	...
	for(size_t i = 0; i < rsize; i++) {
		if (num == ns->numbers[i]) {
			// Поскольку числовому множеству не нужны дубликаты, просто не будем их вставлять
			return NO_ERROR;
            }

		// Для удобства работы с числовым множеством, пусть оно будет отсортированным по возрастанию
		if (num < ns->numbers[i]) {
...
// Добавление во мноежство to элементы из множества for
void addFromSetToSet(NumericSet *to, const NumericSet *from, int *error) {
    assert(to != NULL);
	assert(from != NULL);
	assert(error != NULL);
...
NumericSet* getSubtractionOfSets(NumericSet *A, NumericSet *B, int *error) {
	...
	// Если не все элементы из множества B успели залететь, например если размер B гораздо больше А
	while(iB < sizeB) {
...
void checkNumber(const bool afterLeftSquare, bool *afterComma, bool *afterNumber, int *error) {
...
void checkOperator(const bool afterLeftSquare, int* operatorAmount, int* afterOperator, int* error) {
...
void checkLeftSquareBracket(const int circleBracketsAmount, int *afterOperator, bool *afterLeftSquare, bool* squareInCircle, int *setAmount, int *error) {
...
void checkRightSquareBracket(const bool afterComma, bool *afterLeftSquare, int *error) {
...
void checkLeftCircleBracket(const bool afterLeftSquare, bool *squareInCircle, int* circleBracketsAmount, int *error) {
...
void checkRightCircleBracket(int *circleBracketsAmount, int *afterOperator, int *error) {
...
char* checkString(const char* string, size_t size, int *error) {	
	...
	for (const char* symbol = string; *symbol != '\0'; symbol++) {
		...
		if (isSymbolSpace(*symbol)){} // Если встречен пробел
		else if (isSymbolNumber(*symbol)) { // Если встречена цифра		
			checkNumber(afterLeftSquare, &afterComma, &afterNumber, error); // Если число вне скобок
		}
		else if (isSymbolComma(*symbol)) { // Если символ ','	
			checkComma(&afterNumber, &afterComma, error); // Если запятая не идет после числа
		}
		else if (isSymbolOperator(*symbol)) { // Если обнаружен один из операторов
			checkOperator(afterLeftSquare, &operatorAmount, &afterOperator, error); // Если оператор идет после '['
		}
		else if (isSymbolLeftSquareBracket(*symbol)) { // Если символ '['
			checkLeftSquareBracket(circleBracketsAmount, &afterOperator, &afterLeftSquare, &squareInCircle, &setAmount, error); // Если множество не за оператором
		}
		else if (isSymbolRightSquareBracket(*symbol)) {
			checkRightSquareBracket(afterComma, &afterLeftSquare, error); // Если конец множества идёт сразу после запятой
		}
		else if (isSymbolLeftCircleBracket(*symbol)) {
			checkLeftCircleBracket(afterLeftSquare, &squareInCircle, &circleBracketsAmount, error); // Если круглая скобка внутри квадратной
		}
		else if (isSymbolRightCircleBracket(*symbol)) {			
			checkRightCircleBracket(&circleBracketsAmount, &afterOperator, error); // Если ')' идет первее '('
	...
	// Операторов всегда на 1 меньше чем множеств(например [] U [] ^ []). Если это не так, то значит входные данные не верны
...
NumericSet* applyOPeration(char operator, NumericSet* first, NumericSet* second, int *error) {
...
void getAnswer(char* string)
{
	...
	NumericSet *first = NULL;
    NumericSet *second = NULL;
	...
	while (circleBrackets > 0) {
        ...
        for (size_t i = 0; i < strsize; i++) {
			if (isSymbolLeftCircleBracket(string[i])) { //Если обнаружена ( 
				circleBrackets++;
				begin = i;
			}
			else if (isSymbolRightCircleBracket(string[i])) { //Если обнаружена )

				for (int priority = 0; priority < 3; priority++) {
                    for (char* currentOperator = operations; *currentOperator != '\0'; currentOperator++)
					for (char* letter = string + begin; letter < string + i; letter++) {

						if (isSymbolLeftSquareBracket(*letter)) { //Если обнаружена [
						...
						else if (isSymbolRightSquareBracket(*letter)) { //Если обнаружена ]
							if (number != 0) {
						...
						else if (isSymbolComma(*letter)) { //Если обнаружена ,
						...
						else if (isSymbolNumber(*letter)) { //Если обнаружено число 
							number *= 10;
							number += (*letter - '0');
						}
